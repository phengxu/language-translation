    ...
    for sentence in source_text.split('\n'):
        ...
        for word in sentence.split():
            if word in source_vocab_to_int:
                char_source.append(source_vocab_to_int[word])
        source_id_text.append(char_source)
    ...
    for sentence in target_text.split('\n'):
        ...
        for word in sentence.split():
            if word in target_vocab_to_int:
                char_target.append(target_vocab_to_int[word])
        char_target.append(target_vocab_to_int['<EOS>'])
        target_id_text.append(char_target)
...
def process_decoder_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    #duplicate go symbol for each batch
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob, 
    ...
    def lstm_cell():
        ...
        drop_lstm = tf.contrib.rnn.DropoutWrapper(lstm,keep_prob)
        return drop_lstm
    stacklstm = tf.contrib.rnn.MultiRNNCell([lstm_cell() for _ in range(num_layers)])
    
    output,state =tf.nn.dynamic_rnn(stacklstm,embed,source_sequence_length,dtype=tf.float32)
    return output,state
